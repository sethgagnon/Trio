...
enum MealItemSource {
    ...
    var name: String {
        switch self {
        case .scanned(let food): return food.name
        case .preset(let name, _, _, _): return name
        }
    }
    
    var baseCarbs: Decimal {
        switch self {
        case .scanned(let food): return food.carbsPer100g * food.servingSizeG / 100
        case .preset(_, let carbs, _, _): return carbs
        }
    }
    
    var baseFat: Decimal {
        switch self {
        case .scanned(let food): return food.fatPer100g * food.servingSizeG / 100
        case .preset(_, _, let fat, _): return fat
        }
    }
    
    var baseProtein: Decimal {
        switch self {
        case .scanned(let food): return food.proteinPer100g * food.servingSizeG / 100
        case .preset(_, _, _, let protein): return protein
        }
    }
    
    var servingLabel: String? {
        switch self {
        case .scanned(let food): return food.servingLabel
